...
COREDATA_EXTERN NSString * const NSDetailedErrorsKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);           // if multiple validation errors occur in one operation, they are collected in an array and added with this key to the "top-level error" of the operation

COREDATA_EXTERN NSString * const NSValidationObjectErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);    // object that failed to validate for a validation error
COREDATA_EXTERN NSString * const NSValidationKeyErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);       // key that failed to validate for a validation error
COREDATA_EXTERN NSString * const NSValidationPredicateErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0); // for predicate-based validation, the predicate for the condition that failed to validate
COREDATA_EXTERN NSString * const NSValidationValueErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);     // if non-nil, the value for the key that failed to validate for a validation error

COREDATA_EXTERN NSString * const NSAffectedStoresErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);      // stores prompting an error
COREDATA_EXTERN NSString * const NSAffectedObjectsErrorKey __OSX_AVAILABLE_STARTING(__MAC_10_4, __IPHONE_3_0);     // objects prompting an error

COREDATA_EXTERN NSString * const NSSQLiteErrorDomain __OSX_AVAILABLE_STARTING(__MAC_10_5, __IPHONE_3_0);           // Predefined domain for SQLite errors, value of "code" will correspond to preexisting values in SQLite.
